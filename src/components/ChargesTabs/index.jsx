...
const ChargesTabs = () => {
  ...
  const { addClient, selectedClient, setSelectedClient, paidCharges, setPaidCharges, wonsCharges, setWonsCharges, expectedCharges, setExpectedCharges, setSelected } = useContext(MyContext);
  ...
  async function getCharges() {
    ...
    try {
      ...
      const paid = response.data.debts.filter((charge) => charge.status === "Paga");
      const wons = response.data.debts.filter((charge) => charge.status === "Vencida");
      const expected = response.data.debts.filter((charge) => (charge.status === "Pendente"));
      setDbPaid(paid);
      setDbWon(wons);
      ...
      const expectedN = expected.map((e) => Number(e.values)).reduce((a, b) => a + b);
